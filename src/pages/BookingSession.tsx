...
export default function BookingSession() {
  ...
  const [suggestMode, setSuggestMode] = useState<"location" | "mission">("location");
      ...
      <div className="container mx-auto px-4 py-8 z-10 relative">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold bg-gradient-primary bg-clip-text text-transparent mb-4">
            Book Psychological Support Session
          </h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Connect with qualified space psychology professionals for confidential, mission-focused mental health support
          </p>
        </div>
            ...
            <Card className="mb-6">
              <CardHeader>
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <CardDescription>
                      Choose a space psychology professional based on their specialization in astronaut mental health and availability
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <Button size="sm" variant={suggestMode === "mission" ? "default" : "outline"} onClick={() => setSuggestMode("mission")}>
                      <School className="h-4 w-4 mr-1" /> Specialization
                    </Button>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-4">
                {suggestMode === "location" && (
                  <div className="p-3 border rounded-md">
                    <div className="flex items-center gap-2 mb-3 text-sm text-muted-foreground">
                      <MapPin className="h-4 w-4" /> Mission Support Specialists (top 3)
                    </div>
                ...
                {suggestMode === "mission" && (
                  <div className="p-3 border rounded-md">
                    <div className="flex items-center gap-2 mb-3 text-sm text-muted-foreground">
                      <School className="h-4 w-4" /> Find by Specialization
                    </div>
                    <div className="flex items-center gap-2">
                      <Input placeholder="Enter specialization (e.g., ISS, Mars, Lunar, Deep Space)" value={missionType} onChange={(e) => setMissionType(e.target.value)} />
                    </div>
                    {missionType.trim().length > 0 && (
                      <div className="mt-3 grid md:grid-cols-3 gap-3">
                        {(missionType.trim().toLowerCase() === "iss" || missionType.trim().toLowerCase() === "mars" || missionType.trim().toLowerCase() === "lunar" || missionType.trim().toLowerCase() === "deep space"
                          ...
                          <div key={c.id} className={`p-3 rounded-lg border ${selectedCounselor === c.id ? "border-primary bg-primary/5" : "border-border"}`}>
                            ...
                            <Button className="mt-2 w-full" size="sm" onClick={() => setSelectedCounselor(c.id)}>Use this specialist</Button>
                          </div>
                        ))}
                        {!["iss", "mars", "lunar", "deep space"].includes(missionType.trim().toLowerCase()) && (
                          <div className="text-sm text-muted-foreground col-span-3">No mapped specialists for this specialization yet.</div>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Select value={formData.issue} onValueChange={(value) => setFormData({...formData, issue: value})} required>
                      <SelectTrigger>
                        ...
                        <SelectContent>
                        <SelectItem value="anxiety">Mission Anxiety & Stress</SelectItem>
                        <SelectItem value="depression">Depression & Mood in Space</SelectItem>
                        <SelectItem value="isolation">Isolation & Confinement</SelectItem>
                        <SelectItem value="homesickness">Homesickness & Distance</SelectItem>
                        <SelectItem value="sleep">Sleep Disruption in Space</SelectItem>
                        <SelectItem value="overview">Overview Effect Processing</SelectItem>
                        <SelectItem value="other">Other Space-Related Concerns</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Mission Support Center, Psychological Services</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
